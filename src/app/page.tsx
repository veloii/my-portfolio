import Image from "next/image";
import ProjectPreview from "~/components/ProjectPreview";
import avatar from "~/media/images/avatar/pic.jpg";
import iphone from "~/media/images/projects/condition-perform/iphone1.png";
import continueWithSalu from "~/media/images/projects/salu-mail/continue-with-salu.png";

export default function Home() {
  return (
    <div className="pt-24 relative px-4 lg:px-8 mx-auto text-white z-10">
      <div className="max-w-6xl mx-auto space-y-10">
        <Image
          src={avatar}
          height={89}
          width={89}
          className="rounded-full"
          alt="Veloi"
        />
        <div className="space-y-4">
          <h1 className="font-bold text-4xl tracking-tight">
            Welcome, I&#39;m Veloi.
          </h1>
          <div className="max-w-2xl text-lg tracking-wide text-neutral-400 space-y-4">
            <p>
              I&#39;m a skilled React developer with expertise in TypeScript and
              React Native, dedicated to crafting delightful user experiences.
            </p>
            <p className="text-rose-400 text-base">
              Open for commissions on Fiverr, let&#39;s bring your ideas to
              life!
            </p>
          </div>
          <div className="flex pt-4 gap-5 items-center">
            <a
              href="https://www.fiverr.com/veloi_/create-a-custom-website-with-react"
              target="_blank"
              aria-label="Fiverr"
            >
              <button className="px-4 py-1.5 hover:bg-rose-400 hover:border-transparent hover:text-black hover:[box-shadow:_0_0_16px_0.5px_rgb(251,113,133)] border-neutral-900 text-neutral-200 active:opacity-80 active:scale-95 hover:border-neutral-800 hover:text-white transition duration-300 ease-in-out bg-black rounded-lg border">
                Hire me on Fiverr
              </button>
            </a>
            <a
              href="https://twitter.com/veloi_"
              target="_blank"
              className="group"
              aria-label="Twitter"
            >
              <svg
                viewBox="0 0 24 24"
                aria-hidden="true"
                className="h-6 w-6 transition fill-neutral-400 group-hover:fill-neutral-300"
              >
                <path d="M20.055 7.983c.011.174.011.347.011.523 0 5.338-3.92 11.494-11.09 11.494v-.003A10.755 10.755 0 0 1 3 18.186c.308.038.618.057.928.058a7.655 7.655 0 0 0 4.841-1.733c-1.668-.032-3.13-1.16-3.642-2.805a3.753 3.753 0 0 0 1.76-.07C5.07 13.256 3.76 11.6 3.76 9.676v-.05a3.77 3.77 0 0 0 1.77.505C3.816 8.945 3.288 6.583 4.322 4.737c1.98 2.524 4.9 4.058 8.034 4.22a4.137 4.137 0 0 1 1.128-3.86A3.807 3.807 0 0 1 19 5.274a7.657 7.657 0 0 0 2.475-.98c-.29.934-.9 1.729-1.713 2.233A7.54 7.54 0 0 0 22 5.89a8.084 8.084 0 0 1-1.945 2.093Z"></path>
              </svg>
            </a>
            <a
              href="https://github.com/veloii"
              target="_blank"
              className="group"
              aria-label="Github"
            >
              <svg
                viewBox="0 0 24 24"
                aria-hidden="true"
                className="h-6 w-6 transition fill-neutral-400 group-hover:fill-neutral-300"
              >
                <path
                  fill-rule="evenodd"
                  clip-rule="evenodd"
                  d="M12 2C6.475 2 2 6.588 2 12.253c0 4.537 2.862 8.369 6.838 9.727.5.09.687-.218.687-.487 0-.243-.013-1.05-.013-1.91C7 20.059 6.35 18.957 6.15 18.38c-.113-.295-.6-1.205-1.025-1.448-.35-.192-.85-.667-.013-.68.788-.012 1.35.744 1.538 1.051.9 1.551 2.338 1.116 2.912.846.088-.666.35-1.115.638-1.371-2.225-.256-4.55-1.14-4.55-5.062 0-1.115.387-2.038 1.025-2.756-.1-.256-.45-1.307.1-2.717 0 0 .837-.269 2.75 1.051.8-.23 1.65-.346 2.5-.346.85 0 1.7.115 2.5.346 1.912-1.333 2.75-1.05 2.75-1.05.55 1.409.2 2.46.1 2.716.637.718 1.025 1.628 1.025 2.756 0 3.934-2.337 4.806-4.562 5.062.362.32.675.936.675 1.897 0 1.371-.013 2.473-.013 2.82 0 .268.188.589.688.486a10.039 10.039 0 0 0 4.932-3.74A10.447 10.447 0 0 0 22 12.253C22 6.588 17.525 2 12 2Z"
                ></path>
              </svg>
            </a>
          </div>
        </div>
        <section className="pt-12 pb-4 grid md:grid-cols-2 gap-6">
          <ProjectPreview
            id="sm"
            theme="dark"
            title="Salu Mail"
            description="Enter the secure privacy era"
            className="w-full"
            image={{
              src: continueWithSalu,
              alt: "Continue with Salu",
              height: 400,
            }}
            href="/salu-mail"
          />
          <ProjectPreview
            id="cp"
            theme="dark"
            title="Condition & Perform"
            description="A client project completed on Fiverr"
            className="w-full"
            image={{
              src: iphone,
              alt: "Condition and Perform Site Preview on iPhone",
              className: "pt-4",
              height: 400,
            }}
            hrefExternal
            href="https://stevetunstall.co.uk"
          />
        </section>
      </div>
    </div>
  );
}
